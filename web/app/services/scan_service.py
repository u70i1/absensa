...
def post_scan(db: Session, nisn: str):
    ...
    existing_scan = db.scalar(
        ...
        .where(
            ScanLog.student_id == scanned_student.id,
            ScanLog.timestamp >= start_today,
            ScanLog.timestamp < start_tomorrow,
        )
        .limit(1)
    )
    ...
    if before is not None:
        ...
        stmt = stmt.where(
            or_(
                ...
                and_(
                    ScanLog.timestamp == cursor.timestamp,
                    ScanLog.scan_id < cursor.scan_id,
                ),
            )
        )
    rows = list(
        db.execute(
            stmt.order_by(ScanLog.timestamp.desc(), ScanLog.scan_id.desc()).limit(
                limit + 1
            )
        )
    )
