...
def create_classes_bulk(
    ...
    requested_keys = {
        ...
        for item in payload
        if item.grade is not None and item.class_name is not None
    }
    class_keys_db = set(
        db.execute(
            select(Class.grade, Class.class_name).where(
                tuple_(Class.grade, Class.class_name).in_(requested_keys)
            )
        )
    )
...
def delete_classes_bulk(
    ...
    db_ids = set(
        db.scalars(select(Class.class_id).where(Class.class_id.in_(payload_ids)))
    )
