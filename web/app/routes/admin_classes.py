...
def modal_response(request, template, context, status_code=200):
    return render_modal(
        ...
        context
        | {
            "back_url": "/admin/classes",
            "back_label": "Kembali ke daftar kelas",
            "dialog_title": "Kelas",
        },
        status_code,
    )
    ...
    return modal_response(
        ...
        "modals/message.html",
        {
            ...
            "error": {
                ...
                "duplicate_class": "Nama kelas sudah digunakan pada jenjang ini.",
...
def invalid_filters(request):
    return modal_response(
        ...
        "modals/message.html",
        {
            ...
            "error": "Periksa nilai jenjang pada alamat halaman.",
        },
        422,
    )
...
def render_directory(request, db, query, message=None):
    ...
    response = templates.TemplateResponse(
        request=request,
        name="tables/class-results.html" if fragment else "admin/pages/classes.html",
        context={
            "query": query,
            "classes": class_service.get_class_directory(
                db, query.class_name, query.grade, query.empty
            ),
            "summary": class_service.get_class_summary(db),
            "grades": class_service.get_class_grades(db),
            "message": message,
...
def form_response(request, class_=None, values=None, errors=None, status_code=200):
    return modal_response(
        request,
        "modals/class-form.html",
        {
            "class_": class_,
            "values": values
            if values is not None
            else (
                {"class_name": class_.class_name, "grade": class_.grade}
                if class_
                else {}
            ),
            "errors": errors or {},
...
def save_class(request, db, data, class_id=None):
    ...
    try:
        class_ = (
            class_service.get_class_by_id(db, class_id)
    ...
    try:
        ...
        errors = {
            ...
            }.get(str(error["loc"][0]), "Nilai tidak valid.")
    ...
    try:
        ...
        return form_response(
            ...
            values,
            {
                "form": "Nama kelas sudah digunakan pada jenjang ini."
                if exc.detail == "duplicate_class"
                else exc.detail
            },
            exc.status_code,
        )
...
def update_class(
    class_id: int, request: Request, db: Db, data: dict = Depends(form_data)
):
