...
def render_row_editor(
    request, db, batch, row, *, values=None, error=None, status_code=200, revision=None
):
    if values is None:
    ...
    response = templates.TemplateResponse(
        request=request,
        name="modals/import-row-form.html",
        context={
            "batch": batch,
            "row": row,
            "values": values,
...
def import_page(
    request: Request, db: Db, admin: CurrentAdmin, batch: str | None = None
):
    try:
...
@router.post(
    "/preview/{kind}", name="admin_import_preview_legacy", include_in_schema=False
)
async def preview_import(
    request: Request, db: Db, admin: CurrentAdmin, kind: Kind | None = None
):
    try:
        async with request.form(max_files=1, max_fields=2) as form:
            ...
            if not isinstance(file, UploadFile):
                raise AppException(
                    "Pilih satu file Excel atau arsip terlebih dahulu.", 422
                )
        ...
        return RedirectResponse(
            f"/admin/import?batch={batch.token}#import-review", status_code=303
        )
    except (AppException, HTTPException) as exc:
        message = (
            exc.detail
            if isinstance(exc, AppException)
            else "Unggah tepat satu file Excel atau arsip."
        )
...
def confirm_import(
    ...
    try:
        ...
        return render_import(
            request, batch, exc.detail, exc.status_code, selected_rows=selected
        )
