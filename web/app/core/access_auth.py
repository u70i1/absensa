...
class AccessAuthenticationRequired(Exception):
    def __init__(self, layer: str, destination: str = "/operator"):
        self.layer = layer
        self.location = (
            ("/trusteddevice/login" if layer == "device" else "/operator/login")
            + "?"
            + urlencode({"next": safe_destination(destination)})
        )
...
def require_trusted_device(
    request: Request,
    device: Annotated[TrustedDevice | None, Depends(get_current_trusted_device)],
) -> TrustedDevice:
    if device is None:
