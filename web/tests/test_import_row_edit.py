...
def test_edit_student_draft_then_confirm(
    ...
    response = client.post(
        url, data={**student_values(), "revision": 0, "selected": [2], "id": 999}
    )
...
def test_class_edit_and_revision_conflict(client, importer, db_session, existing_class):
    batch, _ = batch_for(
        preview(client, "classes", [[existing_class.class_id, 10, "Draft"]]),
        client,
        db_session,
    )
    url = f"/admin/import/{batch.token}/rows/2/edit"
...
def test_edit_does_not_refresh_stale_snapshot(
    client, importer, db_session, existing_student
):
    batch, _ = batch_for(
        preview(
...
def test_nisn_edit_retains_staged_photo(client, importer, db_session, monkeypatch):
    ...
    with ZipFile(archive, "w") as output:
        output.writestr(
            "students.xlsx",
            workbook_bytes(
                "students", [[None, "Photo", None, "0012345678", "Aktif", None]]
            ),
        )
        output.writestr("photos/0012345678.png", image.getvalue())
    batch, _ = batch_for(
        preview(client, content=archive.getvalue(), filename="data.zip"),
        client,
        db_session,
    )
    response = client.post(
    ...
    monkeypatch.setattr(
        import_service.student_photo_service, "store_normalized_photo", save_photo
    )
    assert (
        client.post(
            f"/admin/import/{batch.token}/confirm", data={"selected": [2]}
