...
from app.services.exceptions import DuplicateScanLog
...
class TestGetScan:
    ...
    def test_get_scan_date_to_includes_final_fraction_of_second(
        self, client, db_session, student_factory
    ):
        student = student_factory()
        last_moment = make_scan(
            db_session,
            student,
            datetime(2026, 8, 4, 23, 59, 59, 900000, tzinfo=LOCAL_TZ),
        )
        response = client.get("/scans", params={"date_to": "2026-08-04"})
        assert last_moment.scan_id in [row["scan_id"] for row in response.json()]
...
class TestPostScan:
    ...
    def test_scan_at_end_of_local_day_is_still_a_duplicate(
        self, db_session, student_factory, monkeypatch
    ):
        student = student_factory()
        day = datetime(2026, 9, 24, tzinfo=LOCAL_TZ)
        make_scan(
            db_session,
            student,
            day.replace(hour=23, minute=59, second=59, microsecond=500000),
        )

        class FixedDateTime(datetime):
            @classmethod
            def now(cls, tz=None):
                return day.replace(hour=23, minute=59, second=59, microsecond=900000)

        monkeypatch.setattr(scan_service, "datetime", FixedDateTime)
        with pytest.raises(DuplicateScanLog):
            scan_service.post_scan(db_session, student.nisn)
        assert len(scan_logs_for(db_session, student.id)) == 1
...
class TestDeleteScan:
    def test_delete_scan_removes_row(self, client, db_session, existing_student):
