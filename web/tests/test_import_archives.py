...
def upload(client, content, name="data.zip"):
    return client.post(
        "/admin/import/preview", files={"file": (name, content)}, follow_redirects=False
    )
...
@pytest.mark.parametrize(
    "kind,rows", [("students", [student_row()]), ("classes", [[None, 10, "10A"]])]
)
def test_xlsx_detected_without_kind_or_filename_hint(
    client, importer, db_session, kind, rows
):
    batch, _ = batch_for(
...
def test_archive_formats_end_to_end(
    client, importer, db_session, tmp_path, suffix, format_, filter_
):
    files = {
    ...
    batch, page = batch_for(
        upload(client, content, f"file.{suffix}"), client, db_session
    )
...
def test_mixed_workbooks_unique_row_keys_and_optional_photos(
            ...
            zip_bytes(
                {
                    ...
                    "School/photos/": b"",
                    "School/.DS_Store": b"ignored",
                    "__MACOSX/._data.xlsx": b"ignored",
                }
            ),
        ),
        client,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "/absolute.xlsx",
        "C:/file.xlsx",
        "nested/deeper/data.xlsx",
        "photos/not-a-nisn.png",
        "photos/123.png",
        "photos/0012345678.svg",
    ],
)
...
def test_present_invalid_and_unmatched_photos_are_reported(
    client, importer, db_session
):
    batch, page = batch_for(
        upload(
            client,
...
def test_duplicates_across_workbooks_and_duplicate_photos(client, importer, db_session):
    ...
    batch, _ = batch_for(
        upload(client, zip_bytes({"one.xlsx": content, "two.xlsx": content})),
        client,
        db_session,
    )
        ...
        zip_bytes(
            {
                "data.xlsx": content,
                "photos/0012345678.png": photo_bytes(),
                "photos/0012345678.jpg": photo_bytes(),
            }
        ),
    )
    assert response.status_code == 422
    ...
    content = zip_bytes(
        {
            "data.xlsx": workbook_bytes(
                "students", [student_row(), student_row("0012345679")]
            ),
            "photos/0012345678.png": photo_bytes(),
        }
    )
...
def test_existing_photo_preserved_without_replacement(
...
def test_replacement_cleanup_and_stale_photo_protection(
    client, importer, db_session, existing_student
):
...
def test_photo_rollback_if_later_data_write_fails(
    client, importer, db_session, photo_directory, monkeypatch
):
    batch, _ = batch_for(
        upload(
            client,
...
def test_preview_photo_authorization_expiry_and_cleanup(client, importer, db_session):
    ...
    with pytest.raises(AppException):
        import_service.get_preview_photo(
            db_session, importer.id + 1, batch.token, "0012345678"
        )
    token = batch.token
...
def test_global_row_limit_and_bad_workbook_reporting(
    client, importer, db_session, monkeypatch
):
    content = workbook_bytes("classes", [[None, 10, "10A"]])
    batch, page = batch_for(
        upload(client, zip_bytes({"good.xlsx": content, "broken.xlsx": b"bad"})),
        client,
        db_session,
    )
    assert len(batch.payload["rows"]) == 1
    ...
    assert (
        upload(
            client, zip_bytes({"one.xlsx": content, "two.xlsx": content})
        ).status_code
        == 413
    )
