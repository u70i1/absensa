...
class TestCreateClass:
    ...
    def test_create_class_name_too_long_rejected(self, client):
        ...
        response = client.post(
            CLASSES_URL, json=make_class_payload(class_name="X" * 21)
        )
        assert response.status_code in (400, 422)

    def test_create_class_name_at_max_length_accepted(self, client):
        response = client.post(
            CLASSES_URL, json=make_class_payload(class_name="X" * 10)
        )
        assert response.status_code == 201
    ...
    def test_create_class_duplicate_grade_and_name_rejected(
