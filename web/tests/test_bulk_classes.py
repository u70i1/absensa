...
class TestBulkCreateEdgeCases:
    ...
    def test_duplicate_grade_and_name_against_existing_db_row(
        self, client, class_factory
    ):
    ...
    def test_duplicate_class_name_batch_vs_db_have_distinct_errors(
        self, client, class_factory
    ):
    ...
    def test_large_batch_all_valid(self, client, db_session):
        ...
        all_created = db_session.scalars(
            select(Class).where(Class.class_name.like("B%"))
        ).all()
...
class TestBulkUpdate:
    def test_updates_multiple_classes(
...
class TestBulkUpdateEdgeCases:
    def test_one_id_does_not_exist(
        self, client, seeded_students_and_classes, db_session
    ):
    ...
    def test_two_rows_in_batch_swap_class_names(
        self, client, seeded_students_and_classes, db_session
    ):
    ...
    def test_three_way_class_name_rotation(
        self, client, seeded_students_and_classes, db_session
    ):
        ...
        assert (
            len(
                {refreshed_a.class_name, refreshed_b.class_name, refreshed_c.class_name}
            )
            == 3
        )
    ...
    def test_swap_succeeds_even_when_sibling_row_fails(
        ...
        payload = [
            ...
            {"class_id": class_y.class_id, "class_name": original_x_name},
            {
                "class_id": class_z.class_id,
                "class_name": "Way Too Long Name100000000000000",
            },  # broken: exceeds max_length
        ]
...
class TestBulkDelete:
    def test_deletes_multiple_existing_classes(
        self, client, seeded_students_and_classes, db_session
    ):
        classes = db_session.scalars(select(Class)).all()
        to_delete = classes[:2]
...
class TestBulkDeleteEdgeCases:
    def test_one_id_does_not_exist_blocks_entire_batch(
        ...
        response = client.post(
            "/classes/bulk-delete", json={"ids": real_ids + [fake_id]}
        )
    ...
    def test_multiple_missing_ids_all_reported_at_once(
        ...
        response = client.post(
            "/classes/bulk-delete", json={"ids": [real_id] + fake_ids}
        )
    ...
    def test_duplicate_ids_in_same_request(
        ...
        response = client.post(
            "/classes/bulk-delete",
            json={"ids": [target.class_id, target.class_id, classes[1].class_id]},
        )
...
class TestBulkDeleteResponseShape:
    def test_success_response_is_empty_204(
        self, client, seeded_students_and_classes, db_session
    ):
        classes = db_session.scalars(select(Class)).all()
...
class TestBulkDryRun:
    ...
    def test_dry_run_delete_empty_list_is_still_a_204_noop(self, client):
        response = client.post(
            "/classes/bulk-delete", json={"ids": []}, params={"dry_run": True}
        )
