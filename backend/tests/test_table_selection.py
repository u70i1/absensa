...
def admin_client(client, db_session):
    admin = Admin(username='selection-admin', password_hash=hash_password('test-password'))
...
def submit(client, kind, ids, action='delete', step='apply', **kwargs):
    return client.post(f'/admin/{kind}/selection/{step}',
...
def test_student_confirmation_and_subset_delete(admin_client, db_session, student_factory):
    first = student_factory()
    second = student_factory(nisn='1000000002')
    ids = [first.id, second.id]
    preview = submit(admin_client, 'students', [first.id], step='confirm', headers={'HX-Request': 'true'})
    ...
    response = submit(admin_client, 'students', [first.id], headers={'HX-Request': 'true'})
...
def test_deactivate_keeps_student_data(admin_client, db_session, student_factory):
    first = student_factory()
    second = student_factory(nisn='1000000002')
    response = submit(admin_client, 'students', [first.id, first.id], action='deactivate')
    assert response.status_code == 303
...
def test_class_delete_unassigns_students(admin_client, db_session, class_factory, student_factory):
    first = class_factory()
...
def test_missing_record_prevents_partial_change(admin_client, db_session, student_factory):
    student = student_factory()
    response = submit(admin_client, 'students', [student.id, 999999], action='deactivate')
...
def test_invalid_action_and_csrf(admin_client, class_factory):
    cls = class_factory()
    assert submit(admin_client, 'classes', [cls.class_id], action='deactivate').status_code == 422
    assert submit(admin_client, 'classes', [cls.class_id], headers={'Origin': 'https://evil.example'}).status_code == 403
...
def test_class_directory_sorted_by_id(db_session, class_factory):
    ...
    assert [row.class_id for row in get_class_directory(db_session, None, None)] == [first.class_id, second.class_id]
