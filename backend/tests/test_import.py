...
def preview(client, kind="students", rows=None, content=None, filename="data.xlsx"):
    return client.post(
        ...
        files={"file": (filename, content if content is not None else workbook_bytes(kind, rows or []), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")},
...
def test_import_requires_admin(client):
    for path in ("/admin/import", "/admin/import/template/students", "/admin/import/template/classes"):
        assert client.get(path, follow_redirects=False).status_code == 303
    assert preview(client).status_code == 303
    assert client.post("/admin/import/missing/confirm", follow_redirects=False).status_code == 303
...
def test_preview_then_selected_create_update_and_retry(client, importer, db_session, existing_student):
    student = existing_student
    response = preview(client, rows=[
        [student.id, "Nama Baru", student.class_id, student.nisn, "Tidak aktif", "08123456789"],
    ...
    response = client.post(f"/admin/import/{batch.token}/confirm", data={"selected": [2, 3]}, follow_redirects=False)
    assert response.status_code == 303
    db_session.refresh(student)
    assert student.name == "Nama Baru" and student.current is False
    assert student.guardian_phone == "08123456789"
    assert db_session.scalar(select(Student).where(Student.nisn == "0012345678")) is not None
    ...
    assert client.post(f"/admin/import/{batch.token}/confirm", data={"selected": [2, 3]}).status_code == 200
    assert db_session.scalar(select(func.count()).select_from(Student)) == 2


def test_class_import_create_and_update(client, importer, db_session, existing_class):
    response = preview(client, "classes", [[existing_class.class_id, 12, "12Z"], [None, 10, "10X"]])
    batch, page = batch_for(response, client, db_session)
...
@pytest.mark.parametrize("row,cell", [
...
def test_duplicates_and_empty_rows(client, importer, db_session):
    ...
    batch, _ = batch_for(preview(client, rows=[row, [None] * 6, row]), client, db_session)
    assert batch.payload["rows"] == []
...
def test_empty_wrong_template_and_multiple_files(client, importer):
    ...
    assert preview(client, content=workbook_bytes("classes", [[None, 10, "10A"]])).status_code == 422
    response = client.post("/admin/import/preview/students", files=[("file", ("one.xlsx", b"a")), ("file", ("two.xlsx", b"b"))])
...
def test_stale_preview_does_not_overwrite_changes(client, importer, db_session, existing_student):
    batch, _ = batch_for(preview(client, rows=[[existing_student.id, "From file", existing_student.class_id, existing_student.nisn, "Aktif", None]]), client, db_session)
...
def test_atomic_rollback_on_later_write_failure(client, importer, db_session, monkeypatch):
    ...
    assert client.post(f"/admin/import/{batch.token}/confirm", data={"selected": [2, 3]}).status_code == 409
...
def test_tampered_or_empty_selection_cannot_write(client, importer, db_session):
    ...
    for data in ({}, {"selected": [999]}):
        assert client.post(f"/admin/import/{batch.token}/confirm", data=data).status_code == 422
    assert db_session.scalar(select(func.count()).select_from(Class)) == 0
