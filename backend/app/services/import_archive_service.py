...
ARCHIVE_SUFFIXES = (".zip", ".rar", ".7z", ".tar", ".tar.gz", ".tgz", ".tar.bz2", ".tbz2", ".tar.xz", ".txz")
...
def _safe_name(name: str) -> str:
    name = name.replace("\\", "/")
    parts = name.split("/")
    if name.startswith("/") or any(part == ".." or ":" in part or "\x00" in part for part in parts):
...
def _metadata(name: str) -> bool:
    ...
    return "__MACOSX" in path.parts or path.name in {".DS_Store", "Thumbs.db"} or path.name.startswith("~$")


def read_archive(content: bytes) -> tuple[list[tuple[str, bytes]], dict[str, tuple[str, bytes]]]:
    ...
    try:
        with libarchive.memory_reader(content, format_name=format_name) as archive:
            for index, entry in enumerate(archive, 1):
                if index > MAX_ENTRIES:
                    raise AppException("Arsip memuat terlalu banyak file atau folder (maksimal 2.000).", 413)
                name = _safe_name(entry.pathname)
                if entry.linkpath or not (entry.isfile or entry.isdir):
                    raise AppException("Arsip tidak boleh memuat tautan atau file khusus.", 422)
                if entry.isdir:
                    continue
                limit = MAX_IMAGE_BYTES if PurePosixPath(name).suffix.lower() in {".jpg", ".jpeg", ".png", ".webp"} else MAX_WORKBOOK_BYTES
                if entry.size is not None and entry.size > limit:
                    raise AppException(f"File {name} terlalu besar (Excel maksimal 10 MB, foto 5 MB).", 413)
                chunks = bytearray()
                for block in entry.get_blocks(65536):
                    expanded += len(block)
                    if expanded > MAX_EXPANDED_BYTES or len(chunks) + len(block) > limit:
                        raise AppException("Isi arsip terlalu besar setelah dibuka (maksimal 200 MB).", 413)
                    chunks.extend(block)
                if _metadata(name):
                    continue
        ...
        raise AppException("Arsip rusak, terenkripsi, atau format kompresinya tidak didukung. Gunakan arsip tanpa kata sandi.", 422) from exc
    ...
    for name, data in sorted(files.items()):
        ...
        elif len(path.parts) == 2 and path.parts[0].lower() == "photos" and PHOTO_NAME.fullmatch(path.name):
            nisn = path.stem
            if nisn in photos:
                raise AppException(f"Lebih dari satu foto ditemukan untuk NISN {nisn}.", 422)
            photos[nisn] = (name, data)
        else:
            raise AppException(f"Struktur arsip tidak sesuai: {name}. Letakkan file .xlsx di root dan foto photos/NISN.jpg, .png, atau .webp.", 422)
    if not workbooks:
