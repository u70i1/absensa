...
from app.services.admin_auth_service import password_hasher, hash_password, normalize_username
...
def device_for_token(db: Session, token: str | None, *, lock=False):
    if not token or len(token) > 256:
        return None
    stmt = select(TrustedDevice).where(TrustedDevice.binding_hash == token_digest(token), TrustedDevice.active.is_(True))
    if lock:
...
def bind_device(db: Session, username: str, password: str) -> str:
    device = db.scalar(select(TrustedDevice).where(TrustedDevice.username == normalize_username(username)).with_for_update().execution_options(populate_existing=True))
    ...
    if device.binding_hash:
        raise AppException("Akun perangkat sudah terhubung. Minta admin mereset koneksi perangkat sebelum masuk kembali.", 409)
...
def operator_for_token(db: Session, device: TrustedDevice | None, token: str | None):
    if device is None or not token or len(token) > 256:
        return None
    return db.scalar(select(Operator).join(OperatorSession, OperatorSession.operator_id == Operator.id).where(
        OperatorSession.token_hash == token_digest(token), OperatorSession.device_id == device.id,
        OperatorSession.binding_hash == device.binding_hash, Operator.active.is_(True)))
...
def login_operator(db: Session, device_token: str, operator_id: str, pin: str) -> str:
    ...
    if device is None:
        raise AppException("Autentikasi perangkat berakhir. Masuk kembali sebagai perangkat.", 401)
    now = now_utc()
    if device.locked_until and device.locked_until > now:
        raise AppException("Terlalu banyak PIN salah. Perangkat dikunci sementara selama 5 menit.", 429)
    if device.locked_until:
        device.pin_failures = 0
        device.locked_until = None
    valid_id = re.fullmatch(r"[0-9]{1,10}", operator_id or "") and int(operator_id) <= 2147483647
    operator = db.scalar(select(Operator).where(Operator.id == int(operator_id)).with_for_update().execution_options(populate_existing=True)) if valid_id else None
    valid_pin = re.fullmatch(r"[0-9]{6}", pin or "") is not None
    valid = verify_secret(operator.pin_hash if operator else None, pin) if valid_pin else False
    if not valid or operator is None or not operator.active:
        device.pin_failures = min(5, device.pin_failures + 1)
        if device.pin_failures == 5:
            device.locked_until = now + timedelta(minutes=5)
        db.commit()  # Failed attempts must survive the error response.
        raise AppException("Terlalu banyak PIN salah. Perangkat dikunci selama 5 menit." if device.locked_until else "Operator atau PIN tidak valid. PIN harus terdiri dari 6 digit.", 429 if device.locked_until else 401)
    device.pin_failures = 0
    device.locked_until = None
    ...
    db.execute(delete(OperatorSession).where(OperatorSession.device_id == device.id))
    token = token_urlsafe(32)
    db.add(OperatorSession(token_hash=token_digest(token), operator_id=operator.id, device_id=device.id, binding_hash=device.binding_hash))
    db.commit()
    return token
...
def logout_operator(db: Session, device_token: str | None, operator_token: str | None):
    ...
    if device and operator_token:
        db.execute(delete(OperatorSession).where(OperatorSession.device_id == device.id, OperatorSession.token_hash == token_digest(operator_token)))
    db.commit()
