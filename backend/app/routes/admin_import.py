...
def render_import(request, batch=None, error=None, status_code=200, selected_rows=None):
    response = templates.TemplateResponse(
        ...
        context={"batch": batch, "error": error, "selected_rows": selected_rows}, status_code=status_code,
    )
...
@router.post("/preview/{kind}", name="admin_import_preview")
async def preview_import(request: Request, kind: Kind, db: Db, admin: CurrentAdmin):
    try:
        async with request.form(max_files=1, max_fields=2) as form:
            file = form.get("file")
            if not isinstance(file, UploadFile):
                raise AppException("Pilih satu file .xlsx terlebih dahulu.", 422)
        ...
        message = exc.detail if isinstance(exc, AppException) else "Unggah tepat satu file .xlsx."
