...
def directory_response(request, db, message=None):
    ...
    response = templates.TemplateResponse(request=request,
        name='tables/access-results.html' if fragment else 'access-dashboard.html',
        context={'devices': management.directory(db, 'devices'), 'operators': management.directory(db, 'operators'), 'message': message})
    response.headers['Vary'] = 'HX-Request, HX-History-Restore-Request'
    return response


def modal_response(request, template, context, status_code=200):
    return render_modal(request, template, context | {'back_url': '/admin/access', 'back_label': 'Kembali ke akses & perangkat', 'dialog_title': 'Akses & Perangkat'}, status_code)


def form_response(request, kind, item=None, values=None, error=None, status_code=200):
    return modal_response(request, 'modals/access-form.html', {
        'kind': kind, 'item': management.public_account(item) if item else None,
        'values': values if values is not None else (management.public_account(item) if item else {'active': True}),
        'error': error}, status_code)


def error_response(request, exc):
    return modal_response(request, 'modals/message.html', {'title': 'Permintaan tidak dapat diproses', 'error': exc.detail}, exc.status_code)
...
def saved(request, db, message):
    ...
    response.headers.update({'HX-Retarget': '#access-results', 'HX-Reswap': 'outerHTML', 'HX-Trigger-After-Swap': 'accessSaved'})
    return response
...
def save(request, db, admin, kind, data, account_id=None):
    try:
        ...
        db.rollback()
        try:
            item = management.account(db, kind, account_id) if account_id is not None else None
        except AppException:
            return error_response(request, exc)
        # Secrets never enter the template context, including on validation errors.
        return form_response(request, kind, item, {'name': str(data.get('name', ''))[:100], 'active': data.get('active') == 'true'}, exc.detail, exc.status_code)
    return saved(request, db, 'Akun berhasil disimpan.')


@router.post('/{kind}', name='admin_access_create')
def create_account(request: Request, kind: Kind, db: Db, admin: CurrentAdmin, data: dict = Depends(form_data)):
    return save(request, db, admin, kind, data)


@router.post('/{kind}/{account_id:int}/edit', name='admin_access_update')
def update_account(request: Request, kind: Kind, account_id: int, db: Db, admin: CurrentAdmin, data: dict = Depends(form_data)):
    return save(request, db, admin, kind, data, account_id)


@router.get('/{kind}/{account_id:int}/{action}', name='admin_access_confirmation')
def confirmation(request: Request, kind: Kind, account_id: int, action: Literal['revoke', 'delete'], db: Db):
    try:
        ...
        return modal_response(request, 'modals/access-confirm.html', {'kind': kind, 'item': management.public_account(item), 'action': action})
    except AppException as exc:
        return error_response(request, exc)


@router.post('/{kind}/{account_id:int}/{action}', name='admin_access_action')
def account_action(request: Request, kind: Kind, account_id: int, action: Literal['revoke', 'delete'], db: Db):
    try:
        management.manage_account(db, kind, account_id, action)
    except AppException as exc:
        db.rollback()
        return error_response(request, exc)
    return saved(request, db, 'Koneksi perangkat direset. Akun dapat digunakan untuk masuk kembali.' if action == 'revoke' else 'Akun dihapus.')
