...
def post_students_bulk(
    ...
    result = student_bulk_service.create_students_bulk(db, payload)
...
def update_students_bulk(
    ...
    result = student_bulk_service.update_students_bulk(db, payload)
    if result["failed"] and not result["succeeded"]:
        return JSONResponse(status_code=422, content=jsonable_encoder(result))
    return result
...
def delete_students_bulk(payload: BulkStudentIdOnly, db: Session = Depends(get_db)):
    missing_ids = student_bulk_service.delete_students_bulk(db, payload)
    if missing_ids:
        return JSONResponse(
