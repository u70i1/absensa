...
def get_classes(query: Annotated[ClassListQuery, Query()], db: Session = Depends(get_db)):
...
def get_classes_students(
    class_id: int,
    query: Annotated[ClassStudentListQuery, Query()],
...
def post_class(payload: ClassWriteRequest, db: Session = Depends(get_db)):
    """Create new Class item in the database"""
    return class_service.post_class(db, payload.class_name, payload.grade)


@router.put("/classes/{class_id}", response_model=ClassResponse)
def update_class(class_id: int, payload: ClassWriteRequest, db: Session = Depends(get_db)):
