...
DEVICE_COOKIE_SECONDS = 400 * 24 * 60 * 60  # Browser limit; refreshed during authenticated use.
...
def set_device_cookie(response, token):
    ...
    response.set_cookie(DEVICE_COOKIE, token, max_age=DEVICE_COOKIE_SECONDS, path="/",
                        secure=settings.access_cookie_secure, httponly=True, samesite="lax")
...
def clear_access_cookies(response, *, device=False):
    ...
    for name in ([DEVICE_COOKIE, OPERATOR_COOKIE] if device else [OPERATOR_COOKIE]):
        response.delete_cookie(name, path="/", secure=settings.access_cookie_secure, httponly=True, samesite="lax")
...
class BrowserSecurityMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        if request.method not in {"GET", "HEAD", "OPTIONS"}:
            ...
            if (request.headers.get("sec-fetch-site") == "cross-site" or not source
                    or origin_tuple(source) is None or origin_tuple(source) != origin_tuple(str(request.url))):
                return JSONResponse({"detail": "Permintaan harus berasal dari halaman Absensa yang sama. Muat ulang halaman lalu coba lagi."}, status_code=403)
        response = await call_next(request)
        if request.url.path != "/admin/static" and not request.url.path.startswith("/admin/static/"):
